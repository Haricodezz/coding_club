...
own
sync
history\ ON public.leetcode_sync_history FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY \Streaks
are
public\ ON public.leetcode_user_streaks FOR SELECT USING (TRUE);
CREATE POLICY \Ledger
is
public\ ON public.credit_ledger FOR SELECT USING (TRUE);
