...
export default function ContestJoinCard({ contest, onJoin, participantCount = 0, alreadyJoined = false }: Props) {
  const [joining, setJoining] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);

  const handleJoin = async () => {
    if (alreadyJoined || isSuccess) return;
    setJoining(true);
    try {
      const success = await onJoin();
      // If it returned exactly false, don't show the success state.
      // (If it returns undefined/void, assume it succeeded without error)
      if (success !== false) {
        setIsSuccess(true);
      }
    } catch (error) {
  ...
  const isRegistered = alreadyJoined || isSuccess;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem' }}>
      <div style={{ maxWidth: '600px', width: '100%', background: 'var(--color-surface)', borderRadius: '16px', border: '1px solid var(--color-border)', overflow: 'hidden', boxShadow: '0 24px 80px rgba(0,0,0,0.5)' }}>
        ...
        {/* Banner area */}
        <div style={{ height: '120px', background: `linear-gradient(135deg, ${isRegistered ? '#10b981' : 'var(--accent-1)'} 0%, ${isRegistered ? '#059669' : '#3b82f6'} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', transition: 'background 0.5s ease' }}>
        ...
        <div style={{ padding: '2.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2.5rem' }}>
            <div style={{ background: 'var(--color-bg)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--color-border)', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '0.9rem', color: 'var(--text-primary)', fontWeight: 600 }}>{start.toLocaleString()}</span>
            </div>
            <div style={{ background: 'var(--color-bg)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--color-border)', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '0.9rem', color: 'var(--text-primary)', fontWeight: 600 }}>{durationHrs > 0 && `${durationHrs}h `}{durationMins > 0 && `${durationMins}m`}</span>
            </div>
            <div style={{ background: 'var(--color-bg)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--color-border)', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '0.9rem', color: 'var(--text-primary)', fontWeight: 600 }}>{contest.contest_type}</span>
            </div>
            <div style={{ background: 'var(--color-bg)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--color-border)', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '0.9rem', color: 'var(--text-primary)', fontWeight: 600 }}>{participantCount} hackers</span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <button 
              ...
              style={{ width: '100%', padding: '1rem', fontSize: '1.1rem', fontWeight: 700, borderRadius: '10px', background: isRegistered ? '#10b981' : undefined, color: isRegistered ? 'white' : undefined, border: 'none', cursor: isRegistered ? 'default' : 'pointer' }}
            >
              {joining ? 'Registering...' : isRegistered ? '✓ Registered' : 'Join Contest'}
            </button>
