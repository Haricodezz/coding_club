    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
        <div style={{ padding: '1.25rem', background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)' }}>
          ...
          <p style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>{stats?.totalParticipants || 0}</p>
        </div>
        ...
        <div style={{ padding: '1.25rem', background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <p style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>{stats?.submissionsPerMin || 0}</p>
      ...
      {/* Recent Judge Logs */}
      <div style={{ padding: '1.25rem', background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 1rem' }}>Judge System Logs</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {stats.recentLogs.map((log: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '1rem', padding: '0.75rem', background: 'var(--color-bg)', borderRadius: '6px', borderLeft: `3px solid ${log.level === 'ERROR' ? '#ef4444' : '#f59e0b'}` }}>
                ...
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{log.message}</span>
              </div>
            ))}
          </div>
        )}
