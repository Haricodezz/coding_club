...
export default function ProblemPage({
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      channel = supabase.channel(`submissions_${user.id}_${Math.random()}`)
  ...
  if (!problem) return (
    <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg)' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <h2 style={{ color: 'var(--text-primary)' }}>Problem not found</h2>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: 'var(--color-bg)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <PanelGroup direction="horizontal" autoSaveId="contest-workspace-layout-v1">
          
          {/* ── LEFT: Problem Statement ── */}
          <Panel defaultSize={48} minSize={25} maxSize={75} style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--color-bg)' }}>
          ...
          <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)', display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{problem.title}</span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.25rem', padding: '0.6rem 0.85rem', background: 'var(--color-surface)', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
            <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>⏱️ Time: <strong style={{ color: 'var(--text-primary)' }}>{problem.time_limit}ms</strong></span>
            <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>🧠 Memory: <strong style={{ color: 'var(--text-primary)' }}>256 MB</strong></span>
          </div>
          ...
          {problem.input_format && (
            <section style={{ marginBottom: '1.25rem' }}>
              <h4 style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: '0.85rem', margin: '0 0 0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Input Format</h4>
          ...
          {problem.output_format && (
            <section style={{ marginBottom: '1.25rem' }}>
              <h4 style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: '0.85rem', margin: '0 0 0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Output Format</h4>
          ...
          {samples.length > 0 && (
            <section style={{ marginBottom: '1.25rem' }}>
              <h4 style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: '0.85rem', margin: '0 0 0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Examples</h4>
              {samples.map((tc, i) => (
                <div key={tc.id} style={{ marginBottom: '0.75rem', border: '1px solid var(--color-border)', borderRadius: '8px', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0' }}>
                    <div style={{ padding: '0.75rem', borderRight: '1px solid var(--color-border)' }}>
                      ...
                      <pre style={{ fontFamily: 'monospace', fontSize: '0.82rem', color: 'var(--text-secondary)', margin: 0, whiteSpace: 'pre-wrap' }}>{tc.input}</pre>
                    </div>
                    <div style={{ padding: '0.75rem' }}>
                      ...
                      <pre style={{ fontFamily: 'monospace', fontSize: '0.82rem', color: 'var(--text-secondary)', margin: 0, whiteSpace: 'pre-wrap' }}>{tc.expected_output}</pre>
                    </div>
                  </div>
                  {tc.explanation && (
                    <div style={{ padding: '0.6rem 0.75rem', borderTop: '1px solid var(--color-border)', background: 'rgba(108,99,255,0.04)' }}>
                      <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: 0 }}><strong style={{ color: 'var(--accent-1)' }}>Note:</strong> {tc.explanation}</p>
                    </div>
                  )}
                </div>
              ))}
